	...
	if ($locale) {
		...
		$locale, $matches)) {
			...
			if ($modifier) {
				...
				elseif ($charset) array_push($locale_names, "${lang}.$charset@$modifier");
			...
			elseif ($charset) array_push($locale_names, "${lang}.$charset");
			array_push($locale_names, $lang);
		}

		// If the locale name doesn't match POSIX style, just include it as-is.

		if ( ! in_array($locale, $locale_names)) array_push($locale_names, $locale);
	}
